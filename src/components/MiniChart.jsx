...
export default function MiniChart({ data, label, color, suffix = "", showZeroLine = false, invert = false, compact = false }) {
  ...
  const W = 300, H = compact ? 50 : 90, PAD = 24;
  const plotTop = compact ? 10 : 14, plotBot = compact ? 10 : 14;
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: H }}>
      ...
      {points.map((p, i) => (
        <g key={i}>
          ...
          <text x={p.x} y={p.y - 7} textAnchor="middle" fill={color} fontSize={compact ? "8" : "9"} fontWeight="800">{p.value}{suffix}</text>
          <text x={p.x} y={H - 1} textAnchor="middle" fill="#475569" fontSize={compact ? "6" : "7"} fontWeight="600">{p.label}</text>
        </g>
      ))}
    </svg>
  );
